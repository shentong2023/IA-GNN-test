...
class IAGNN(Module):
    ...
    def forward(self, data, opt):  # (batch, max_len) pad的元素会对结果有影响吗？需不需要mask？
        ...
        # length = torch.sum(mask, dim=1)  # (batch)
        ...
        # m = e_v
        # print('m:', m.size(), m)
        A = nn.functional.normalize(v, dim=2)  # l2 norm
        A = torch.matmul(A, A.permute(0, 2, 1))
        H = torch.where(A > 0, torch.FloatTensor([1]).cuda(), torch.FloatTensor([0]).cuda())
        B = torch.sum(H, dim=2)  # (batch, max_len)
        B = torch.where(B > 0, 1.0 / B, B)
        m = torch.matmul(B.unsqueeze(2) * H, item_emb)  # (batch, max_len(n_i), emb_size)
        ...
        alpha = torch.matmul(alpha, self.q_alpha.unsqueeze(1))  # (batch, max_len(n_i), 1)
        # print('alpha:', alpha.size(), alpha)
        # (batch, n_intentions, emb_size) (emb_size, 1) => (batch, n_intentions, 1)
        # beta = alpha
        zero_vec = -9e15 * torch.ones_like(alpha).cuda()
        # mask注意力系数
        attention = torch.where(mask.unsqueeze(-1) > 0, alpha, zero_vec)
        beta = F.softmax(attention, dim=1)  # (batch, max_len(n_i), 1)
