...
class GraphAttentionLayer(nn.Module):
    def __init__(self, opt):
        ...
        self.dropout = 0.3
        self.w_1 = nn.Parameter(torch.randn(opt.emb_size, opt.emb_size))
        self.a = nn.Parameter(torch.randn(2 * opt.emb_size, 1))

    def forward(self, item_emb, adj):
        Wh = torch.matmul(item_emb, self.w_1)
        a_input = self._prepare_attentional_mechanism_input(Wh)
        e = F.leaky_relu(torch.matmul(a_input, self.a).squeeze(3))
        ...
        # 注意力系数加权求和
        item_emb = torch.matmul(attention, Wh)
    ...
    def _prepare_attentional_mechanism_input(self, Wh):
        ...
        return all_combinations_matrix.reshape(Wh.size(0), N, N, -1)
...
class IAGNN(Module):
    ...
    def forward(self, data, opt):  # (batch, max_len) pad的元素会对结果有影响吗？需不需要mask？
        ...
        A = torch.matmul(A, A.permute(0, 2, 1))
        H = torch.where(A > 0, torch.FloatTensor([1]).cuda(), torch.FloatTensor([0]).cuda())
        v = self.GAT(item_emb, H)  # (batch, max_len, emb_size)
        # v = item_emb
