...
parser.add_argument('--dataset', default='yoochoose', help='dataset name:diginetica/tmall/retailrocket')
...
def main():
    ...
    train_data = pickle.load(open('/home/huashanshan/IA-GNN/datasets/yoochoose/preprocessed/train.txt', 'rb'))
    test_data = pickle.load(open('/home/huashanshan/IA-GNN/datasets/yoochoose/preprocessed/test.txt', 'rb'))
    # train_data = pickle.load(open('/home/huashanshan/DHCN/datasets/Tmall/train.txt', 'rb'))
    # test_data = pickle.load(open('/home/huashanshan/DHCN/datasets/Tmall/test.txt', 'rb'))

    if opt.dataset == 'diginetica':
        n_node = 43097  # 实际node个(不加补充的node)
    elif opt.dataset == 'tmall':
        # n_node = 43061  # 实际node个(不加补充的node) 我的数据
        n_node = 40727  # 不含 DHCN数据
    elif opt.dataset == 'retailrocket':
        n_node = 48983  # 实际node个(不加补充的node)
    elif opt.dataset == 'nowplaying':
        n_node = 31418  # 实际node个(不加补充的node)
    elif opt.dataset == 'yoochoose':
        n_node = 6823 # 7290  # 实际node个(不加补充的node)
    ...
    for k in top_k:
        best_result['hit@%d' % k] = 0
        best_result['mrr@%d' % k] = 0
        best_result['ndcg@%d' % k] = 0
        best_result['epoch%d' % k] = [0, 0, 0]
    bad_count = 0
    ...
    for epoch in range(opt.epoch):
        ...
        for k in top_k:
            ...
            if metric['mrr@%d' % k].item() > best_result['mrr@%d' % k]:
                ...
                if epoch != 0:
                    bad_count = 0
                best_result['epoch%d' % k][1] = epoch
            if metric['ndcg@%d' % k].item() > best_result['ndcg@%d' % k]:
                best_result['ndcg@%d' % k] = metric['ndcg@%d' % k].item()
                flag = 0
                if epoch != 0:
                    bad_count = 0
                best_result['epoch%d' % k][1] = epoch
        ...
        for k in top_k:
            print('recall@%d:%.4f\t mrr@%d:%.4f\t ndcg@%d:%.4f'
                  % (k, metric['hit@%d' % k].item(), k, metric['mrr@%d' % k].item(), k, metric['ndcg@%d' % k].item()))
        print('best_result----')
        for k in top_k:
            print('recall@%d:%.4f\t mrr@%d:%.4f\t ndcg@%d:%.4f\t epoch:%d,%d,%d'
                  % (k, best_result['hit@%d' % k], k, best_result['mrr@%d' % k], k, best_result['ndcg@%d' % k], best_result['epoch%d' % k][0], best_result['epoch%d' % k][1], best_result['epoch%d' % k][2]))
        if bad_count >= opt.patience:
            break
